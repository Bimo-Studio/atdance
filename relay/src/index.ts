 ...
 */
import { loadRelayInviteContext } from './allowlistStore';
...
export type { RelayWorkerEnv } from './workerEnv';
...
export default {
  async fetch(request: Request, env: RelayWorkerEnv): Promise<Response> {
    if (request.headers.get('Upgrade') !== 'websocket') {
      return handleRelayHttp(request, env);
    }
      ...
      void (async () => {
        ...
        if (parsed !== null) {
          ...
          if (parsed.type === 'joinQueue') {
            ...
            const invite = await loadRelayInviteContext(env.ALLOWLIST_KV, env);
          ...
          const r = applyRelayMessage(relayState, senderId, parsed, undefined);
          relayState = r.state;
          for (const e of r.effects) {
            sendToClient(e.toClientId, e.message);
          }
